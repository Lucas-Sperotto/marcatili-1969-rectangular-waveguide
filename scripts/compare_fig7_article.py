...
"""Build a side-by-side comparison between the Figure 7 scan and the current reproduction."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a side-by-side comparison between the Figure 7 article scan and the current reproduction."
    )
    parser.add_argument(
        "plot_png",
        type=Path,
        help="Generated PNG from scripts/plot_fig7.py.",
    )
...
def build_canvas(article: Image.Image, plot: Image.Image) -> Image.Image:
    ...
    draw.text((margin, margin), "Figure 7 article scan", fill="black")
    draw.text((margin + article.width + gap, margin), "Figure 7 current reproduction", fill="black")
...
def main() -> None:
    ...
    article = trim_white_border(Image.open(repo_root / "docs/img/fig_07.png").convert("RGB"))
