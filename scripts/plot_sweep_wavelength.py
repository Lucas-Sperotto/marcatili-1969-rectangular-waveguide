...
"""Plot kz vs wavelength from reproduce_sweep_wavelength CSV output."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot kz vs wavelength from reproduce_sweep_wavelength CSV output."
    )
    parser.add_argument(
        "csv_file",
        nargs="?",
        type=Path,
        default=Path("data/output/sweep_wavelength.csv"),
        help="CSV produced by reproduce_sweep_wavelength (default: data/output/sweep_wavelength.csv).",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def load_data(
    ...
    with csv_file.open(newline="", encoding="utf-8") as handle:
        ...
        required = {"wavelength", "mode_family", "p", "q", "kz_rad_per_m"}
        ...
        for row in reader:
            ...
            xs, ys = series[key]
            xs.append(float(row["wavelength"]) * 1e6)  # convert m → µm
            ys.append(float(row["kz_rad_per_m"]))
