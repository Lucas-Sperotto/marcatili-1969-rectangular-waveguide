...
1. **A Arquitetura:** Entenda o fluxo de dados (`JSON` → C++ → `CSV` → Python). Veja o `docs/10_fluxo_geral_do_repositorio.md` e preste atenção nos padrões de projeto, como *Pipeline* e *Strategy*.
2. **O Algoritmo Central:** O coração do solver `exact` é um algoritmo de **busca de raízes**. Estude o `src/math/root_finding.cpp` e a discussão em `docs/11_closed_form_vs_exact.md`. É um exemplo prático de análise numérica.
3. **A Separação de Camadas:** Observe como o código é organizado em `physics`, `math`, `io` e `apps`. Essa separação de responsabilidades é crucial para a manutenibilidade de qualquer software.

Depois de entender *como* o software funciona, a física se tornará mais um "domínio de aplicação" interessante do que uma barreira.
...
- **Analogia para CS:** Pense no índice de refração como o **custo** ou **peso** de uma aresta em um grafo. A luz (ou um algoritmo de caminho mínimo como Dijkstra) sempre buscará o caminho de menor "custo" total. Para a luz ficar confinada, o "custo" do caminho fora do guia deve ser maior.
...
- **Analogia para CS:** É uma **simplificação do modelo** para tornar o problema computacionalmente tratável. Similar a assumir que um grafo é pouco denso para que um algoritmo funcione de forma mais eficiente, ou usar uma aproximação em série de Taylor e desprezar termos de ordem superior.
...
- **Analogia para CS:** Um modo guiado é como um **estado estável** em um sistema dinâmico ou um **autovetor** de uma matriz de transformação. É uma configuração específica que, uma vez estabelecida, se mantém enquanto se propaga pelo sistema (o guia). O objetivo do solver é encontrar esses autovetores.
...
- **Analogia para CS:** É uma **violação de restrição** em um problema de otimização. Abaixo do cutoff, a "solução" (o modo) não é mais válida ou viável de acordo com as regras do sistema físico.
...
- **Analogia para CS:** É a "distância de influência" de um nó. Pense em um algoritmo de PageRank, onde a influência de uma página decai à medida que você se afasta dela. Aqui, a influência do campo decai exponencialmente para fora do núcleo.
...
- **Analogia para CS:** São os **autovalores** do problema. As condições de contorno (as "paredes" do guia) restringem `kx` e `ky` a um conjunto discreto de valores permitidos, assim como as dimensões de uma matriz restringem seus autovalores. O problema central do solver é encontrar esses autovalores.
...
- **Analogia para CS:** É uma forma de **normalização de dados**. Ao reescalar as dimensões do problema usando uma "régua" natural, os resultados se tornam mais gerais e comparáveis, independentemente das unidades absolutas. É como normalizar features para um algoritmo de machine learning.
...
- **Analogia para CS:** É um **algoritmo de tempo constante O(1)** que usa uma fórmula analítica. É uma heurística rápida baseada em uma simplificação do modelo.
...
- **Analogia para CS:** É um **algoritmo iterativo** (como a bisseção, com complexidade **O(log N)**) que encontra uma solução numérica precisa. É o contraponto da heurística `closed_form`.
...
**Navegação:** [Anterior](14_diagramas_de_fluxo_e_sequencia.md) | [Índice](00_resumo.md) | [Checklist](09_checklist_reproducao.md) | [Roteiro](15_roteiro_de_estudo.md) | [Próximo](20_auditoria_tecnica_do_modelo.md)
