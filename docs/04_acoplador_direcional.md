...
=
2\,\frac{k_x^2}{k_z a}\,
\frac{\xi_5\,\exp(-c/\xi_5)}{1+k_x^2\xi_5^2}.
\tag{33}
$$

Para modos $E^{x}_{pq}$, $k_x$, $k_z$ e $\xi_5$ são dados nas equações (3) e (8), e $k_x$ é a solução da equação (6). De modo semelhante, para modos $E^{y}_{pq}$, $\mathbf{k}_x$, $\mathbf{k}_z$ e $\boldsymbol{\xi}_5$ são obtidos das equações (17), (18) e (20). Como esperado, o acoplamento decresce exponencialmente com a razão $c/\xi_5$, entre a separação dos guias e a penetração de campo no meio 5.
...
$$
=
2\left(\frac{k_xA_5}{\pi}\right)^2
\left[
1-\left(\frac{k_xA_5}{\pi}\right)^2
\right]^{1/2}
\exp\left\{
-\pi\frac{c}{A_5}
\left[
...
**Fig. 9 —** Acoplador direcional imerso em vários dielétricos:  
(a) seção transversal;  
(b), (c) e (d) distribuições de campo.
...
### Fig. 10 — Coeficiente de acoplamento para modos $E^{x}_{11}$

Legenda:  
**Fig. 10 —** Coeficiente de acoplamento para modos $E^{x}_{11}$.  
— acoplamento calculado a partir das equações transcendentais;  
— aproximações em forma fechada;  
— acoplamento entre duas hastes cilíndricas (A. L. Jones).

### Fig. 11 — Coeficiente de acoplamento para modos $E^{y}_{11}$

Legenda:  
**Fig. 11 —** Coeficiente de acoplamento para modos $E^{y}_{11}$.  
— acoplamento $E^{y}_{11}$ para $n_1/n_5=1.5$;  
— acoplamento $E^{x}_{11}$ para $n_1/n_5=1.1$.
...
\exp\left\{
-\pi
\left(\frac{n_1^2}{n_5^2}-1\right)^{-1}
\frac{c\delta}{A_5}
\left[
1-\left(\frac{2}{\pi}+\frac{a}{A_5}\right)^{-2}
\right]^{1/2}
\right\}.
\tag{36}
$$
...
- Mantive a organização do texto em torno das Figs. 9, 10 e 11, porque esta seção mistura explicação física, comparação gráfica e exemplos numéricos.
...
- Seção **IV. Directional Coupler**;
- equações (33) a (37);
- discussão das Figs. 9, 10 e 11;
- exemplos numéricos de comprimento de acoplamento, separação mínima e modulação por variação de índice.
