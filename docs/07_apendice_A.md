...
$$
H_{xv} = e^{-ik_z z+i\omega t} \begin{cases}
...
$$
E_{xv} = -\frac{1}{\omega\varepsilon n_\nu^2 k_z} \frac{\partial^2 H_{xv}}{\partial x \partial y},
$$
...
$$
E_{zv} = \frac{i}{\omega\varepsilon n_\nu^2} \frac{\partial H_{xv}}{\partial y}.
$$
...
$$
k_{x\nu}^2+k_{y\nu}^2+k_z^2 = \omega^2\varepsilon\mu\,n_\nu^2 = k_\nu^2.
\tag{39}
$$
...
as componentes de campo $E_x$ na equação (38) podem ser desprezadas.
...
$$
\tan\left(\frac{k_y b}{2}\pm\beta\right) = i \frac{n_1^2}{n_{2,4}^2}\,\frac{k_{y\,2,4}}{k_y},
\tag{44}
$$

$$
\tan\left[k_x\left(a+\frac{c}{2}\right)+\alpha\right] = -\,i\,\frac{k_{x5}}{k_x}\, i\cot\left(k_{x5}\frac{c}{2}+\gamma\right).
\tag{45}
$$
...
$$
k_x a  = k_{x0}a \left[ 1+ \frac{2\xi_5}{a} \, \frac{\exp\!\left(-\frac{c}{\xi_5}-2i\gamma\right)} {1+k_{x0}^2\xi_5^2} \right],
\tag{48}
$$
...
$$
k_{x0}a = p\pi - \tan^{-1}(k_{x0}\xi_3) - \tan^{-1}(k_{x0}\xi_5),
\tag{49}
$$

$$
\eta_{2,4} = \frac{1}{|k_{y2,4}|} = \left[ \left(\frac{\pi}{A_{2,4}}\right)^2-k_y^2 \right]^{-1/2},
\tag{50}
$$

$$
\xi_{3,5} = \frac{1}{|k_{x3,5}|} = \left[ \left(\frac{\pi}{A_{3,5}}\right)^2-k_{x0}^2 \right]^{-1/2},
\tag{51}
$$
...
$$
A_{2,3,4,5} = \frac{\pi}{\left(k_1^2-k_{2,3,4,5}^2\right)^{1/2}} = \frac{\lambda}{2\left(n_1^2-n_{2,3,4,5}^2\right)^{1/2}}.
\tag{52}
$$
...
$$
\begin{cases} k_{zs} \\ k_{za} \end{cases} = k_{z0} \left[ 1 \pm \frac{2k_{x0}^2\xi_5}{a k_{z0}^2} \frac{\exp(-c/\xi_5)} {1+k_{x0}^2\xi_5^2} \right].
\tag{54}
$$
...
$$
k_{z0} = \left(k_1^2-k_{x0}^2-k_y^2\right)^{1/2}
\tag{55}
$$

é a constante de propagação do modo $E^{y}_{pq}$ de um guia único $(c\rightarrow\infty)$.
...
$$
\begin{aligned}
-iK = \frac{\pi}{2L} = \frac{k_{zs}-k_{za}}{2} &= 2\,\frac{k_{x0}^2\xi_5}{k_{z0}a} \frac{\exp(-c/\xi_5)} {1+k_{x0}^2\xi_5^2} \\
&= 2\,\frac{A_5 k_{x0}^2}{\pi a k_{z0}} \left[ 1-\left(\frac{k_{x0}A_5}{\pi}\right)^2 \right]^{1/2} \exp\left\{ -\pi\frac{c}{A_5} \left[ 1-\left(\frac{k_{x0}A_5}{\pi}\right)^2 \right]^{1/2} \right\}.
\end{aligned}
\tag{56}
$$
...
$$
\left(\frac{k_yA_{2,4}}{\pi}\right)^2 \ll 1.
\tag{58}
$$
...
## A.2 Modos $E^{x}_{pq}$: polarização na direção $x$
...
$$
-iK = \frac{\pi}{2L} = 2\,\frac{\mathbf{k}_{x0}^{\,2}\boldsymbol{\xi}_5}{\mathbf{k}_{z0}a} \frac{\exp(-c/\boldsymbol{\xi}_5)} {1+\left(\mathbf{k}_{x0}\boldsymbol{\xi}_5\right)^2},
\tag{59}
$$
...
$$
\mathbf{k}_{z0} = \left(k_1^2-\mathbf{k}_{x0}^{\,2}-\mathbf{k}_y^{\,2}\right)^{1/2},
\tag{60}
$$
...
$$
\mathbf{k}_y b = q\pi - \tan^{-1}(\mathbf{k}_y\boldsymbol{\eta}_2) - \tan^{-1}(\mathbf{k}_y\boldsymbol{\eta}_4),
\tag{61}
$$
...
$$
\mathbf{k}_{x0} a = p\pi - \tan^{-1}\!\left(\frac{n_3^2}{n_1^2}\mathbf{k}_{x0}\boldsymbol{\xi}_3\right) - \tan^{-1}\!\left(\frac{n_5^2}{n_1^2}\mathbf{k}_{x0}\boldsymbol{\xi}_5\right),
\tag{62}
$$
...
$$
\boldsymbol{\eta}_{2,4} = \left[ \left(\frac{\pi}{A_{2,4}}\right)^2-\mathbf{k}_y^{\,2} \right]^{1/2},
\tag{63}
$$
...
$$
\boldsymbol{\xi}_{3,5} = \left[ \left(\frac{\pi}{A_{3,5}}\right)^2-\mathbf{k}_{x0}^{\,2} \right]^{1/2}.
\tag{64}
$$
...
$$
\left(\frac{\mathbf{k}_{x0}A_{3,5}}{\pi}\right)^2 \ll 1,
\tag{65}
$$
...
$$
\left(\frac{\mathbf{k}_yA_{2,4}}{\pi}\right)^2 \ll 1.
\tag{66}
$$
...
**Navegação:** [Anterior](06_resumo_e_conclusoes.md) | [Índice](00_resumo.md) | [Checklist](09_checklist_reproducao.md) | [Roteiro](15_roteiro_de_estudo.md) | [Próximo](08_referencias.md)
